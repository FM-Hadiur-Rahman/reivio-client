...
const MapboxAutocomplete = ({
  ...
  const defaultCenter = useMemo(
    () => initialCoordinates || [90.4125, 23.8103],
    // IMPORTANT: only for first init center; updates handled separately below
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [],
  );
  ...
  // ---- debounce (stable)
  const debounceRef = useRef(null);
  const debounce = useCallback((fn, ms = 300) => {
    if (debounceRef.current) clearTimeout(debounceRef.current);
    debounceRef.current = setTimeout(fn, ms);
  }, []);

  const reverseGeocode = useCallback(async (lng, lat, labelText = "") => {
    try {
      setBusy(true);
      const res = await fetch(
        `https://api.mapbox.com/geocoding/v5/mapbox.places/${lng},${lat}.json?access_token=${mapboxgl.accessToken}`,
      ...
      const data = await res.json();
      const addr = labelText || data?.features?.[0]?.place_name || "";
      setAddress(addr);
  ...
  const ensureMarker = useCallback(() => {
    ...
    if (!markerRef.current) {
      markerRef.current = new mapboxgl.Marker({ draggable: true }).addTo(map);
  ...
  }, [debounce, reverseGeocode]);

  const moveTo = useCallback(
    (coords, labelText = "", fly = true) => {
      const map = mapRef.current;
      if (!map) return;
      ...
      debounce(() => reverseGeocode(lng, lat, labelText), 250);
    },
    [debounce, ensureMarker, reverseGeocode],
  );
  ...
  useEffect(() => {
    ...
    map.on("load", () => {
      ...
      // initial marker if provided at mount
      if (initialCoordinates?.length === 2) {
        moveTo(initialCoordinates, initialAddress || "", false);
        return;
      }

      // optional GPS centering (no marker)
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          (pos) => {
            const { longitude, latitude } = pos.coords;
  ...
  // ✅ IMPORTANT FIX: react to prop updates WITHOUT recreating map
  useEffect(() => {
    if (!ready) return;
    if (!initialCoordinates || initialCoordinates.length !== 2) return;

    // Update marker/map when parent changes location later
    moveTo(initialCoordinates, initialAddress || "", true);
  }, [ready, initialCoordinates, initialAddress, moveTo]);

  // Keep address state in sync if parent changes only address
  useEffect(() => {
  ...
  return (
    <div className="relative">
      <div
        ref={mapContainerRef}
        className="w-full h-[300px] sm:h-[420px] rounded-3xl overflow-hidden border border-slate-200 shadow-sm"
        style={{ minHeight: "300px" }}
      />
